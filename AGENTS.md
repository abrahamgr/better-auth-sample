...
Runtime config is validated in `apps/web/app/lib/env.server.ts` using Zod. All variables are required with no fallbacks — the app will throw at startup if any are missing. Copy `apps/web/.env.sample` to `apps/web/.env` and fill in real values before running. Do not commit `.env`; it is gitignored. Generate `BETTER_AUTH_SECRET` with `openssl rand -base64 32`. `RESEND_API_KEY` is the only optional variable — omit it in development and the app falls back to SMTP.
